"""Scrollable in-window image viewer with bounded background resize work."""
import io
import math
import queue
import threading
...
def limits(size, bounds):
    width, height = size
    maximum = min(8, 8192 / width, 8192 / height,
                  math.sqrt(16_000_000 / (width * height)))
    fit = min(1, bounds[0] / width, bounds[1] / height, maximum)
...
class ImageZoom(tk.Frame):
    def __init__(self, parent, path, thumbnail, title, style, close, copy=None):
        ...
        self.worker = threading.Thread(target=self.decode, args=(path, self.tasks, self.results, self.stop),
                                       daemon=True, name='image-zoom')
        self.worker.start()
        self.place(x=0, y=0, relwidth=1, relheight=1)
        self.poll_timer = self.after(20, self.poll)
        # Opening the viewer is an explicit click: keyboard shortcuts work here,
        # then exit returns focus to the application the user was working in.
    ...
    def zoom(self, factor, point=None):
        ...
        self.focal = (max(0, min(1, (self.canvas.canvasx(x) - self.offset[0]) / self.photo.width())),
                      max(0, min(1, (self.canvas.canvasy(y) - self.offset[1]) / self.photo.height())), x, y)
    ...
    @staticmethod
    def decode(path, tasks, results, stop):
        source = None
        try:
            while not stop.is_set():
                ...
                try:
                    if source is None:
                        with Image.open(path) as original: source = original.convert('RGBA')
    ...
    def destroy(self):
        ...
        for sequence, binding in self.key_bindings: self.root.unbind(sequence, binding)
        ...
        elif self.previous_focus and self.previous_focus.winfo_exists(): self.previous_focus.focus_force()
        super().destroy()
