...
"""
    ...
    def test_external_image_capture_thumbnail_zoom_and_explicit_copy(self):
        ...
        self.root.event_generate('<Control-c>');self.root.update()
        self.assertEqual(viewer.copy_button.cget('text'),'Copied')
        self.root.event_generate('<Control-Shift-C>');self.root.update()
        self.assertEqual(self.root.clipboard_get(),str(self.store.images / item['image']))
