...
class UiTests(unittest.TestCase):
    ...
    def test_switch_files_above_document_without_returning_to_history(self):
        ...
        self.window.activate_item(item)
        self.wait(lambda:self.window.current is not None)
        self.assertFalse(hasattr(self.window, 'file_tabs') and self.window.file_tabs.isVisible())
        self.assertEqual(self.window.folder_button.text(), f'📁 {self.path.name} ▾')
        self.window.load_path(second)
        ...
        self.window.load_path(first)
        self.wait(lambda:self.window.current.path==first)
        self.window.source_toggle.setChecked(True);self.window.refresh_view()
